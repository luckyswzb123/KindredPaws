...
export default function Applications() {
  ...
  const handleStatusChange = async (id: string, status: 'approved' | 'rejected') => {
    const confirmMsg = status === 'approved' ? '确认批准该申请？用户将收到通知并可进行领养流程。' : '确定要拒绝该申请吗？';
  ...
  return (
    <div className="space-y-12">
      <header className="flex flex-col gap-2">
        ...
        <p className="text-on-surface-variant text-base font-light font-sans italic opacity-80">在此处理来自所有用户的宠物领养或寄养申请。批准后，系统将自动向用户通过 APP 发送站内信。</p>
      </header>
      ...
      <div className="grid grid-cols-1 gap-8">
        {isLoading ? (
          <div className="py-40 text-center flex flex-col items-center gap-6">
             ...
             <span className="font-headline font-bold text-on-surface-variant/40 uppercase tracking-widest text-sm italic">申请数据加载中...</span>
          </div>
        ) : apps.length === 0 ? (
          ...
          apps.map((app) => (
            <div key={app.id} className="bg-white p-10 rounded-3xl business-shadow border border-outline-variant grid grid-cols-1 lg:grid-cols-12 gap-10 items-center group hover:border-accent transition-all animate-in fade-in slide-in-from-bottom-5">
              <div className="lg:col-span-3 flex items-center gap-6">
                ...
                <div className="flex flex-col min-w-0">
                  <h3 className="font-headline font-bold text-xl text-on-surface tracking-tight truncate">{app.user_profiles?.name || app.applicant_name}</h3>
                  <span className="text-[10px] text-on-surface-variant font-bold uppercase tracking-[0.2em] mt-2 font-mono">{app.applicant_phone}</span>
                </div>
              </div>
              ...
              <div className="lg:col-span-3 flex flex-col gap-2.5">
                ...
                <div className="flex items-center gap-4 bg-accent/5 p-3 rounded-[20px] border border-accent/10 pr-6">
                   <div className="w-12 h-12 rounded-xl bg-white overflow-hidden border border-outline-variant shrink-0 business-shadow p-0.5 flex items-center justify-center">
                      <img src={app.pets?.image_url} className="w-full h-full object-cover rounded-lg" alt={app.pets?.name} />
                   </div>
                   <div className="flex flex-col min-w-0">
                      <span className="font-headline font-bold text-sm text-accent truncate tracking-tight">{app.pets?.name || '未知宠物'}</span>
                      <span className="text-[10px] text-accent/60 italic font-bold uppercase tracking-widest mt-0.5">{app.pets?.breed || '未知品种'}</span>
                   </div>
                </div>
              </div>
              ...
              <div className="lg:col-span-4 flex flex-col gap-3 p-6 bg-surface-container-low/50 rounded-2xl border border-outline-variant/30 backdrop-blur-sm">
                ...
                <div className="flex gap-2">
                   <span className="text-[9px] px-2 py-1 bg-white border border-outline-variant rounded-lg font-bold text-on-surface-variant uppercase tracking-widest">{app.housing_type}</span>
                   <span className="text-[9px] px-2 py-1 bg-white border border-outline-variant rounded-lg font-bold text-on-surface-variant uppercase tracking-widest">{app.experience_level}经验</span>
                </div>
              </div>
                  ...
                  <div className={cn(
                    ...
                    {app.status === 'approved' ? (
                      <>
                        ...
                        <span className="font-headline font-bold text-[10px] uppercase tracking-widest">领养申请已获批</span>
                      </>
                    ) : (
                      <>
